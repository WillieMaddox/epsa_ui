...
const result = (function () {
  ...
  return {
    init: function (geom_type, label) {
      console.log(geom_type, 'component measure.init() called')
      //TODO: Understand why can't we set the format measure function here?
      //I think this problem is related to import
      //Probably need to use constructor functions and create "measure" objects with "new"
      // measureFunc = measureFuncFormats[geom_type]

      return this.createNode(label)
    },
    destroy: function (removeComponent) {
      console.log('destroy', removeComponent)
    },
    // measureFunc: function measureFuncFormats[geom_type],
    createNode: function (label) {
      const $formElem = $("<div class='form-elem'>")
      const $formValue = $("<div class='form-value'>")
      $formElem.append($('<div id="measure-label" class="form-label">' + label + '</div>'))
    ...
    activateNode: function (feature, measureFunc) {
